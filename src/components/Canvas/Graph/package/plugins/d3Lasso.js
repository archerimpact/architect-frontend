...
export function setupLasso() {
    d3.lasso = function() {
        ...
        function dragmove() {
            ...
            // If the path is closed and close select is set to true, draw the closed paths and count edges
             if(isPathClosed === true && closePathSelect === true) {
                ...
                for (let i = 0; i <= close_path_length; i++) {
                    ...
                    items[0].filter((d) => { return d.lassoPoint.cy == Math.round(cur_pos.y); }).forEach((d) => {
                ...
                items[0].forEach(function(a) {
                    a.loopSelected = (a.lassoPoint.edges.left + a.lassoPoint.close_edges.left) > 0 
            ...
            // Tag possible items
            d3.selectAll(items[0].filter((d) => { return (d.loopSelected && isPathClosed) || d.hoverSelected; }))
                .each((d) => { d.possible = true;});
        ...
        lasso.on = function(type,_) {
            ...
            if (types.indexOf(type) > -1) { on[type] = _; }
            return lasso;
        };
        ...
        lasso.enable = function(_) {
            enable = true;
            return lasso;
        }

        lasso.disable = function(_) {
            enable = false;
            return lasso;
        }
