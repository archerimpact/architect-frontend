...
export function styleNode(selection) {
  ...
  .attr('r', (d) => {
    return d.radius = (d.group ? constants.GROUP_NODE_RADIUS : constants.NODE_RADIUS);
  })
  .classed('hull-node', (d) => {
    return d.group;
  })
  .style('stroke', (d) => {
    return d.group ? colors.HEX_WHITE : getNodeColor(d);
  })
  .style('fill', (d) => {
    return d.group ? getNodeColor(d) : colors.HEX_LIGHT_GRAY;
  });
...
export function styleLink(selection, isSelected) {
  ...
  .style('stroke-width', (l) => {
    return (l.source.group && l.target.group ? constants.GROUP_STROKE_WIDTH : constants.STROKE_WIDTH) + 'px';
  })
...
export function fillGroupNodes() {
  ...
  .classed('grouped', function (d) {
    return isGroup(d) || d.type === 'same_as_group';
  });
}
