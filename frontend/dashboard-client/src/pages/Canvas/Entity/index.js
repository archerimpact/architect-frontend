    ...
    return (
      <div>
        ...
        <div className="entity-body">
          ...
          {aliases.map(a => <EntityCard data={a} shouldFetch/> )}

          {documents.length ? <h5>Documents</h5> : null}
          {documents.map(a => <EntityCard data={a} shouldFetch/> )}

          {maybe_sames.length ? <h5>Possibly Same As</h5> : null}
          {maybe_sames.map(a => <EntityCard data={a} shouldFetch/> )}

          {others.length ? <h5>Associations</h5> : null}
          {others.map(a => <EntityCard data={a}shouldFetch/>)}
        </div>
      </div>
    )
  }
